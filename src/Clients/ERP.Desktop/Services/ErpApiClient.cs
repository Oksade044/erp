...
public sealed class ErpApiClient(HttpClient http)
{
    ...
    public async Task<(bool ok, string? error)> DeleteSupplierAsync(Guid id, CancellationToken ct = default) =>
        await ReadResultAsync(await http.DeleteAsync($"/api/v1/suppliers/{id}", ct), ct);
    public async Task<(bool ok, string? error)> DeleteUserAsync(Guid id, CancellationToken ct = default) =>
        await ReadResultAsync(await http.DeleteAsync($"/api/v1/users/{id}", ct), ct);
    ...
    /// <summary>Məhsulun anbarlar üzrə mövcudluğu (#18/#19 — rezerv/kirayə/boş).</summary>
    ...
    public Task<List<CustomerReportRowDto>?> GetCustomerReportAsync(CancellationToken ct = default) =>
        http.GetFromJsonAsync<List<CustomerReportRowDto>>("/api/v1/reports/customers", JsonOpts, ct);
